...
class GestorServ:
    ...
    def cargaGestorServ(self, servicio):
    ...
    def buscarServicio(self, nombre, gestorHos):
        i = 0
        while i < self.__cantidadOcupadas and self.__ndarray[i].getNombre() != nombre:
            i += 1
        if i < self.__cantidadOcupadas:
            codH = self.__ndarray[i].getCodH()
            gestorHos.buscarHosPorCod(codH)
        else:
            print("Servicio no encontrado.")

    def acumuladorServHos(self,codHospital):
        acumulador = 0
        for servicio in self.__ndarray:
            if servicio.getCodigoH() == codHospital:
                acumulador += 1
        return acumulador                 